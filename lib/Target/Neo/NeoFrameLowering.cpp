...
{
  const NeoSubtarget &Subtarget = MF.getSubtarget<SparcSubtarget>();
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const NeoRegisterInfo *RegInfo = Subtarget.getRegisterInfo();
  const NeoMachineFunctionInfo *FuncInfo = MF.getInfo<NeoMachineFunctionInfo>();
...
bool NeoFrameLowering::isLeafProc(MachineFunction &MF) const
{
...
void NeoFrameLowering::remapRegsForLeafProc(MachineFunction &MF) const 
{
    return; 
}
